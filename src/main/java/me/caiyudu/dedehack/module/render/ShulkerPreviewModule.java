...
import me.caiyudu.dedehack.events.render.EventRenderTooltip;
...
public final class ShulkerPreviewModule extends Module
{
            ...
            public void run()
            {
                ...
                if (shulkerNBT != null)
                {
                    ...
                    if (SavedShulkerItems.containsKey(customName))
                        ...
                        DeDeHack.SendMessage("New shulker found with name " + customName + " it contains " + l_SlotsNotEmpty + " slots NOT empty");
    ...
    private Listener<EventNetworkPacketEvent> PacketEvent = new Listener<>(p_Event ->
    {
        ...
        else if (p_Event.getPacket() instanceof SPacketSetSlot)
        {
            ...
            if (l_Packet.getWindowId() == EnderChestWindowId)
            {
                // dedehack.INSTANCE.logChat("Ender Chest updated, reopen to update preview.");
            }
        }
    });
    ...
    private Listener<EventRenderTooltip> OnRenderTooltip = new Listener<>(p_Event ->
    {
        ...
        if (Item.getIdFromItem(p_Event.getItemStack().getItem()) == 130)
        {
            ...
            // text
            RenderUtil.drawStringWithShadow("DeDeHack EnderChest Viewer", 0, -mc.fontRenderer.FONT_HEIGHT - 1,
                    0xFFFFFFFF);
            ...
            for (int i = 0; i < EnderChestItems.size(); ++i)
            {
                ...
                // dedehack.INSTANCE.logChat("Item: " + itemStack.getDisplayName());
            ...
            if (this.middleClick.getValue())
            {
                if (Mouse.isButtonDown(2))
                {
                    if (!this.clicked)
                    {
                        InventoryBasic l_Inventory = new InventoryBasic("DeDeHack EnderChest Viewer", true, 27);
