    ...
    private Listener<EventNetworkPacketEvent> PacketEvent = new Listener<>(p_Event ->
    {
        if (p_Event.getPacket() instanceof SPacketSpawnMob)
        {
            ...
            if (this.slimes.getValue())
            {
                ...
                if (packet.getEntityType() == 55 && packet.getY() <= 40 && !mc.world.getBiome(mc.player.getPosition()).getBiomeName().toLowerCase().contains("swamp"))
                {
                    ...
                    DeDeHack.SendMessage("Slime Spawned in chunk X:" + Math.round(mc.world.getChunk(pos).x) + " Z:" + Math.round(mc.world.getChunk(pos).z));
                }
            }

            if (this.Donkey.getValue() && packet.getEntityType() == 31)
            {
                DeDeHack.SendMessage(String.format("Donkey spawned at %s %s %s", Math.round(packet.getX()), Math.round(packet.getY()), Math.round(packet.getZ())));
            } else if (this.Llama.getValue() && packet.getEntityType() == 103)
            {
                DeDeHack.SendMessage(String.format("Llama spawned at %s %s %s", Math.round(packet.getX()), Math.round(packet.getY()), Math.round(packet.getZ())));
            } else if (this.Mule.getValue() && packet.getEntityType() == 32)
            {
                DeDeHack.SendMessage(String.format("Mule spawned at %s %s %s", Math.round(packet.getX()), Math.round(packet.getY()), Math.round(packet.getZ())));
            }
        }
        else if (p_Event.getPacket() instanceof SPacketSoundEffect)
        {
            ...
            if (this.thunder.getValue())
            {
                if (packet.getCategory() == SoundCategory.WEATHER && packet.getSound() == SoundEvents.ENTITY_LIGHTNING_THUNDER)
                {
                    ...
                    DeDeHack.SendMessage("Lightning spawned X:" + Minecraft.getMinecraft().player.posX + " Z:" + Minecraft.getMinecraft().player.posZ + " Angle:" + yaw);
                }
            }
        }
        else if (p_Event.getPacket() instanceof SPacketEffect)
        {
            ...
            if (packet.getSoundType() == 1023 && Wither.getValue())
            {
                ...
                DeDeHack.SendMessage("Wither spawned in direction " + angle + " with y position: " + packet.getSoundPos().getY());
                // dedehack.INSTANCE.logChat("Wither spawned at " + packet.getSoundPos().toString() + "You currently are at: X:" + Minecraft.getMinecraft().player.posX + " Z:" +
                // Minecraft.getMinecraft().player.posZ);
            }
            else if (packet.getSoundType() == 1038 && EndPortal.getValue())
            {
                DeDeHack.SendMessage("End portal spawned at " + packet.getSoundPos().toString());
            }
            else if (packet.getSoundType() == 1028 && EnderDragon.getValue())
            {
                DeDeHack.SendMessage("Ender dragon died at " + packet.getSoundPos().toString());
            }
        }
    });
}
