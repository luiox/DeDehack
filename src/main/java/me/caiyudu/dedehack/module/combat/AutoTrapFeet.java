...
public class AutoTrapFeet extends Module
{
    ...
    public void onEnable()
    {
       ...
       //         .orElse(null);
        ...
        if (Target != null)
        {
            DeDeHack.SendMessage("[AutoTrapFeet]: Found target " + Target.getName());
        }
    }
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (IsCurrentTargetTrapped())
        {
            ...
            {
                toggle();
                DeDeHack.SendMessage(ChatFormatting.LIGHT_PURPLE + "[AutoTrapFeet]: Current target is trapped. Toggling");
            }
            return;
        }
        ...
        if (hasStack(Blocks.OBSIDIAN) || slot != -1)
        {
            if ((mc.player.onGround && playerSpeed <= 0.005f))
            {
                ...
                for (BlockPos l_Pos : l_Array)
                {
                    ...
                    {
                    //    DeDeHack.SendMessage(String.format("Can't place at %s because of %s", l_Pos.toString(), l_ResultPlace.toString()));
                    }
                }
