...
import me.caiyudu.dedehack.util.SalUtil;
...
public final class AutoTrap extends Module
{
    ...
    public void onEnable()
    {
        ...
        {
            DeDeHack.SendMessage(String.format("[AutoTrap] You do not have any %s in your hotbar!", ChatFormatting.LIGHT_PURPLE + (EChests.getValue() ? "Ender Chests" : "Obsidian") + ChatFormatting.RESET));
            toggle();
        }
    }
    ...
    public void onDisable()
    {
        ...
        if (announceUsage.getValue())
            DeDeHack.SendMessage("[AutoTrap] Disabled!");
    }
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (closestTarget == null)
        {
            if (firstRun)
            {
                ...
                if (announceUsage.getValue())
                {
                    DeDeHack.SendMessage("[AutoTrap] Enabled, waiting for target.");
                }
            }
            return;
        }
        if (firstRun)
        {
            ...
            if (announceUsage.getValue())
            {
                DeDeHack.SendMessage("[AutoTrap] Enabled, target: " + lastTickTargetName);
            }
        }
        else if (!lastTickTargetName.equals(closestTarget.getName()))
        {
            ...
            if (announceUsage.getValue())
            {
                DeDeHack.SendMessage("[AutoTrap] New target: " + lastTickTargetName);
            }
        }
