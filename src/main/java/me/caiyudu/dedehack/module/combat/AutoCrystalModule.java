...
import me.caiyudu.dedehack.module.misc.AutoMendArmorModule;
import me.caiyudu.dedehack.util.BlockInteractionHelper;
import me.caiyudu.dedehack.util.BlockInteractionHelper.ValidResult;
import me.caiyudu.dedehack.util.CrystalUtils;
import me.caiyudu.dedehack.util.RotationSpoof;
...
public class AutoCrystalModule extends Module
{
    ...
    private BlockPos HandlePlaceCrystal(@Nullable EventPlayerMotionUpdate p_Event) throws Exception
    {
        ...
        if (m_HoleToFill != null)
        {
            DeDeHack.INSTANCE.logChat("Filling the hole at " + m_HoleToFill.toString() + " will deal " + l_DMG);
           // return;
        }*/
       ...
       // DeDeHack.INSTANCE.logChat(String.format("%s%s DMG and SelfDMG %s %s %S", ChatFormatting.LIGHT_PURPLE, l_DMG, l_SelfDMG, l_Facing, m_Target.getName()));
    ...
    private void SpoofRotationsTo(float p_Yaw, float p_Pitch)
    {
        ...
        if (PlayerUtil.isCurrentViewEntity())
        {
            ...
            AxisAlignedBB axidedeignedbb = mc.player.getEntityBoundingBox();
            double l_PosXDifference = mc.player.posX - mc.player.lastReportedPosX;
            double l_PosYDifference = axidedeignedbb.minY - mc.player.lastReportedPosY;
            ...
            {
                mc.player.connection.sendPacket(new CPacketPlayer.PositionRotation(mc.player.posX, axidedeignedbb.minY, mc.player.posZ, l_Yaw, l_Pitch, mc.player.onGround));
            }
            else if (l_MovedXYZ)
            {
                mc.player.connection.sendPacket(new CPacketPlayer.Position(mc.player.posX, axidedeignedbb.minY, mc.player.posZ, mc.player.onGround));
            }
            ...
            if (l_MovedXYZ)
            {
                ...
                mc.player.lastReportedPosY = axidedeignedbb.minY;
