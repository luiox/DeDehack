package me.caiyudu.dedehack.preset;
...
public class Preset
{
    ...
    {
        File exists = new File("DeDeHack/Presets/" + directory.getName() + "/" + directory.getName() + ".json");
        if (!exists.exists())
            return;
        ...
        try 
        {
            ...
            Reader reader = Files.newBufferedReader(Paths.get("DeDeHack/Presets/" + directory.getName() + "/" + directory.getName() + ".json"));
        ...
        try (Stream<Path> paths = Files.walk(Paths.get("DeDeHack/Presets/" + directory.getName() + "/Modules/")))
        {
            paths
                ...
                .forEach(path -> 
                {
                    try 
                    {
                        ...
                        Reader reader = Files.newBufferedReader(Paths.get("DeDeHack/Presets/" + directory.getName() + "/Modules/" + path.getFileName().toString()));
    ...
    public void save()
    {
        try
        {
            ...
            Writer writer = Files.newBufferedWriter(Paths.get("DeDeHack/Presets/" + _displayName + "/" + _displayName + ".json"));
        ...
        try
        {
            ...
            for (Entry<String, ConcurrentHashMap<String, String>> entry : _valueListMods.entrySet())
            {
                ...
                Writer writer = Files.newBufferedWriter(Paths.get("DeDeHack/Presets/" + _displayName + "/Modules/" + entry.getKey() + ".json"));
                Map<String, String> map = new HashMap<>();
