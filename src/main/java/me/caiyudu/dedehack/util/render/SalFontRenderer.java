...
public class SalFontRenderer
{
    ...
    {
        if (name == "Tw Cen MT")
            return getFontFromInput("/assets/dedehack/fonts/tcm.TTF");
        if (name == "Tw Cen MT Std")
            return getFontFromInput("/assets/dedehack/fonts/TwCenMTStd-Bold.ttf");
        if (name == "Verdana Bold")
            return getFontFromInput("/assets/dedehack/fonts/verdanabold.ttf");
        if (name == "Lucida Console")
            return getFontFromInput("/assets/dedehack/fonts/lucdia-console.ttf");
        if (name == "VerdanaBold")
            return getFontFromInput("/assets/dedehack/fonts/verdanabold.ttf");
        
        // Attempt to find custom fonts
        return Font.createFont(Font.TRUETYPE_FONT, new File(DirectoryManager.Get().GetCurrentDirectory() + "\\DeDeHack\\Fonts\\" + name + ".ttf"));
    }
